...
void Gps::update(){
  ...
  if ( this->updateTimeLast > millis() ){ this->updateTimeLast = millis(); }
  double td = (double)( millis() - (this->updateTimeLast) )/1000.0d;
  
  if( td >= this->pingTime && runGPS){
    // update timer
    td = pingTime;
    this->updateTimeLast = millis();
...
void BatteryMonitor::update(){
   ...
   */
  double vOut = analogRead(this->batteryPin)/1023 ;                                           // read from pin. Gets values 0-1023. Map 0-1023 to 0.0-1.0 (a normalized scale)
  vOut *= 5.0d;                                                                               // map from 0.0-1.0 to 0.0-5.0 (this is the output Voltage) (0 to 5 volts)
  ...
  double range = ( ( (this->fullVoltage) - (this->criticalVoltage) )* 0.25d ) + (this->criticalVoltage);
  
  // update battery state
  if( (this->batteryVoltage) <= (this->criticalVoltage) ){    // is voltage critical
    this->batteryState = 2;
...
void QuadController::calculate( double dt ){
  ...
  if( desiredRollRate   >   this->maxRotationRateRoll   ){  desiredRollRate   =   this->maxRotationRateRoll;  }
  if( desiredRollRate   <   -this->maxRotationRateRoll   ){  desiredRollRate   =   -this->maxRotationRateRoll;  }

  double desiredPitchRate =   desiredPitch    *   (this->pitchRotationRateScalar);
  if( desiredPitchRate  >   this->maxRotationRatePitch  ){  desiredPitchRate  =   this->maxRotationRatePitch; }
  if( desiredPitchRate  <   -this->maxRotationRatePitch  ){  desiredPitchRate  =   -this->maxRotationRatePitch; }

  double desiredYawRate   =   desiredYaw      *   (this->yawRotationRateScalar);
  if( desiredYawRate    >   this->maxRotationRateYaw    ){  desiredYawRate    =   this->maxRotationRateYaw;   }
  if( desiredYawRate    <   -this->maxRotationRateYaw    ){  desiredYawRate    =   -this->maxRotationRateYaw;   }
...
void QuadController::updateMotorThrust(){
  ...
  double fl = (this->thrust) - (*this->rollOutput) + (*this->pitchOutput) - (*this->yawOutput);
  double fr = (this->thrust) + (*this->rollOutput) + (*this->pitchOutput) + (*this->yawOutput);
  double bl = (this->thrust) - (*this->rollOutput) - (*this->pitchOutput) + (*this->yawOutput);
  double br = (this->thrust) + (*this->rollOutput) - (*this->pitchOutput) - (*this->yawOutput);
